/* Deterministic smoke tests for the Svetlana runtime. */
...
const pose=R.pose('aa',1);
assert.strictEqual(pose.jawOpen,0.6);assert.strictEqual(pose.mouthOpen,0.72);
...
assert(S.get()==='idle');S.transition('thinking');S.transition('speaking');S.transition('interrupted');S.transition('idle');assert(S.get()==='idle');
